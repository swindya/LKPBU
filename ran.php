...
<table style="text-align: left; margin-left: 20px; margin-right: auto; margin-top: auto;" width="900px" border="0" cellspacing="0" cellpadding="0">
	...
	<form name="myrun" id="myrun" action="ran.php" method="post" enctype="multipart/form-data">
...
# query the users table for name and surname
	$query = "SELECT * FROM maindata WHERE (MONTH(tgl)=" . $bulanku . " AND YEAR(tgl)=" . $tahunku .
			" AND (UPPER(instrumenttype) <> 'MISCELLANEOUS' OR UPPER(instrumenttype) <> 'TIME DEPOSIT') AND NOT(depository LIKE '%VAULT%') AND marketvalue <> 0 " .
...
// Baca SandiSecurity -> hapus '.', '-'. spaces
		$a = 0;
		$sql0 = "SELECT * FROM sandisecurity;";
 ...
 /* determine number of rows result set */
		...
		for ($a=1; $a<=$jmla; $a++)
		{
			$shortnamekuku = str_replace('PT','',$shortnamemu[$a]);
			$shortnamekuku = str_replace(' ','',$shortnamekuku);
			$shortnamekuku = str_replace('.','',$shortnamekuku);
			$shortnamekuku = str_replace('-','',$shortnamekuku);
			$shortnamekuku = str_replace(',','',$shortnamekuku);
			$sql1 = "UPDATE sandisecurity SET shortnameclean='" . $shortnamekuku . "' WHERE ID=" . $idmu[$a] . ";";
...
//----------------------------------------------------------------------------
		...
		for ($j=1; $j <= $jmldatamain; $j++)
		{
			...
			$sandinegaraku[$j] = "oo";
			$issuernameku[$j] = "ooo";
			$startdateku[$j] = "oooooooo";
			$enddateku[$j] = "oooooooo";
			$curku[$j] = "xxx";
			$jenissuratberhargaku[$j] = "";
			$penerbitsuratberhargaku[$j] = "xx";
			$issuernameku[$j] = "nonenonenonenonenonenonenonenonenon";

		//$query = "SELECT * FROM sandisecurity WHERE (UPPER(shortname) LIKE '%" . strtoupper($instrumentcodeku[$j]) . "%' OR UPPER(localcode) LIKE '%" .
		//		strtoupper($instrumentcodeku[$j]) . "%');";
			$query = "SELECT * FROM sandisecurity WHERE (UPPER(shortname)='" . strtoupper($instrumentcodeku[$j]) . "' OR UPPER(shortnameclean)='" . 
					$instrumentcodemu[$j] . "');";
//echo $query . "<br>";
			$row_cnt = 0;
			$namaperus = "";
 ...
 /* determine number of rows result set */
			...
			//$statusdataok[$j] = 1;
				...
				//$curku[$j] = $row['cur'];									//IDR
					...
					$sandipenerbitku[$j] = $row['sandipenerbit'];
					$penerbitsuratberhargaku[$j] = $jenissuratberhargaku[$j];
					$sandinegaraku[$j] = $row['negaraasal'];
					$typeku = $row['type'];				//Equities, Bond, Time Deposit
					$subtypeku = $row['subtype'];		//
					$issuernameku1 = $row['issuername'];
//echo $issuernameku1 . "<br>";
			//$issuernameku[$i] = $issuernameku1;
					...
					$startdate = $row['tglterbit'];		//
					if (strlen($startdate)>5) {
						$startarr = explode('-',$startdate);
						$startdateku[$j] = $startarr[2] . $startarr[1] . $startarr[0];
					}
					else
						$startdateku[$j] = "thththth";

					$enddate = $row['tgljatuhtempo'];		//
					if (strlen($enddate)>5) {
						$endarr = explode('-',$enddate);
						$enddateku[$j] = $endarr[2] . $endarr[1] . $endarr[0];
					}
					else
						$enddateku[$j] = "thththth";
				}
			}
//echo  $instrumentcodeku[$j] . "==" . $penerbitsuratberhargaku[$j] . "==" . $query . "<br>";

//CLIENT
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
//update kolom clientnameclean yg berisi data clientname tanpa karakter '.', '-', ',', <space> dan 'Dana Penisun' -> 'DP'
			//$sql0 = "UPDATE client SET shortnameclean=replace(replace(replace(replace(shortname,'.',''),',',''),' ',''),'-','');";
			...
			$result = mysqli_query($link, $sql0);
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
			$clientnamekuku = str_replace('PT','',$clientnameku[$j]);
			$clientnamekuku = str_replace(' ','',$clientnamekuku);
			$clientnamekuku = str_replace('.','',$clientnamekuku);
			$clientnamekuku = str_replace('-','',$clientnamekuku);
			$clientnamekuku = str_replace(',','',$clientnamekuku);
			$clientnamekuku = str_replace('DANAPENSIUN','DP',strtoupper($clientnamekuku));
			
			$query = "SELECT * FROM client WHERE (UPPER(fullname)='" . strtoupper($clientnameku[$j]) . 
					"' OR UPPER(shortnameclean) = '" . strtoupper($clientnamekuku) . "');";
if ($j > 20000)
{
echo $query . "<br>";
}
 ...
 /* determine number of rows result set */
			...
			$row_cnt = 0;
			if (strlen((string)$sandigolku[$j])==1) {
				$sandigolku[$j] = '00' . $sandigolku[$j];
			}
			else if (strlen((string)$sandigolku[$j])==2) {
				$sandigolku[$j] = '0' . $sandigolku[$j];
			}
...
//echo $n . "--" . (string)$sandiku[$n] . $space1 . $sandipemilikku[$n] . $sandipenerbitku[$n] . $issuernameku[$n] . $sandipemilikku[$n] . $space2 . "<br>";
					$numi++;
					$txt = (string)$sandigolku[$n] . $space1 . $sandipemilikku[$n] . $sandipenerbitku[$n] . $issuernameku[$n] . $sandinegaraku[$n] . $space2 . $penerbitsuratberhargaku[$n] .
...
// Change the file
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F3', 'GOLONGAN');
		$objPHPExcel->getActiveSheet()->setCellValue('G3', 'NEGARA ASAL');
		$objPHPExcel->getActiveSheet()->setCellValue('H3', 'VALUTA');
		$objPHPExcel->getActiveSheet()->setCellValue('I3', 'LKPBU DATA');
...
//echo "JmlTotData: " . $jmltotdata . "<br>";
		...
		for ($n=1; $n<=$jmldatamain;$n++) {
				if ($statusdataok[$n] == 1) {
					...
					//if (strlen($issuernameku[$n]) > 3) {
						$txta = (string)$sandigolku[$n] . $space1 . $sandipemilikku[$n] . $sandipenerbitku[$n] . $issuernameku[$n] . $sandinegaraku[$n] . $space2 . $jenissuratberhargaku[$n] .
							$space1 . $jenisvalutaku[$n] . $startdateku[$n] . $enddateku[$n] . $marketvalueku[$n] . $cashproceedku[$n] . "\n";
            ...
            //      $space1 . $jenisvalutaku[$n] . $startdateku[$n] . $enddateku[$n] . $marketvalueku[$n] . $cashproceedku[$n] . "\n";
						...
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(4, $n+3, $penerbitsuratberhargaku[$n]);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(5, $n+3, $sandigolku[$n]);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(6, $n+3, $sandinegaraku[$n]);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(7, $n+3, $jenisvalutaku[$n]);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(8, $n+3, $txta);
