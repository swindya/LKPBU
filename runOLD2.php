...
# query the users table for name and surname 
	$query = "SELECT * FROM maindata WHERE (MONTH(tgl)=" . $bulanku . " AND YEAR(tgl)=" . $tahunku . 
			" AND (UPPER(instrumenttype) <> 'MISCELLANEOUS' OR UPPER(instrumenttype) <> 'TIME DEPOSIT') AND depository <> 'VAULT' AND marketvalue <> 0 " .
...
//----------------------------------------------------------------------------
	for ($j=1; $j <= $jmldatamain; $j++)
	{
		...
		$query = "SELECT * FROM sandisecurity WHERE (UPPER(shortname)='" . strtoupper($instrumentcodeku[$j]) . "');";
		$row_cnt = 0;
		$namaperus = "";
		$result = mysqli_query($link, $query);
		if ($result) {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------		
		...
		//$query = "SELECT * FROM client WHERE (UPPER(shortname)='" . strtoupper($clientnameku[$j]) . "');";
		$row_cnt = 0;
		$tipeinvestorku[$j] = "";
		$sandipemilikku[$j] = "ID";
		$sandigolku[$j] = "000";
...
//echo $query . "--" . $sandigolku[$j] . "--" . $sandipemilikku[$j] . "<br>";
			}
		}
		...
		for ($n=1; $n<=$jmldatamain;$n++) {
				if ($statusdataok[$n] == 1) {
					...
					//if (strlen($issuernameku[$n]) > 3) {
						...
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(4, $n+3, $jenissuratberhargaku[$n]);
