			...
			//{
				...
				if (strlen($clientnameku[$j])>1)
				{
					...
					if ($countarr > 4)
					{
						...
						$query1 = "SELECT * FROM client WHERE (UPPER(fullname) LIKE '%" . strtoupper($clix) . "%' AND LENGTH(sandiasuransi)>1);";
					...
					else if ($countarr == 1)
					{
						...
						$query1 = "SELECT * FROM client WHERE (UPPER(fullname) = '" . strtoupper($clientnameku[$j]) . "' AND LENGTH(sandiasuransi)>1);";
					}
				}
				$row_cnt = 0;
				$statusx = 0;
