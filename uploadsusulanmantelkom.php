...
//---------------------------------------------------------------------------------------------------
//Bersihkan SPPD pada tabel telkomdatindo

$sql = "SELECT * FROM telkomdatindo WHERE sppd LIKE '%TLKM%';";
$row_cnt = 0;
$jmlo = 0;
...
for ($j=1; $j<=$jmlo; $j++)
{
	$sppdarr = explode(" ", $sppdcu[$j]);
	$sppdini = $sppdarr[2];
	$sqlmu = "UPDATE telkomdatindo SET sppd='" . $sppdini . "' WHERE ID=" . $tidcu[$j] . ";";
	$result = mysqli_query($link, $sqlmu);
}

//Query dulu...apakah data sudah ada di database
$sql0 = "SELECT * FROM telkomsusulan WHERE (sppd='" . $sppd . "' AND tahun=" . $tahun . " AND batch=" . $batch . ");";
//echo $sql0 . "<br>";
$row_cnt = 0;
...
if ($row_cnt == 0) {
	$sql00 = "SELECT * FROM telkomdatindo WHERE (sppd='" . $sppd . "' AND tahun=" . $tahun . " AND batchtahun=" . $batch . " AND jenisbayar='TUN');";
//echo $sql00 . "<br>";
	$jmlcnt = 0;
	$bniidku = 0;
 ...
 /* determine number of rows result set */
	...
	$sql = "INSERT INTO telkomsusulan(perusahaanID, telkomdatindoID, nama, tahun, batch, biaya, sppd, nominal, bank, cabang, atasnama, nosurat, jenisesop, tglsusul, norekkd,
			createduser, createddate) VALUES(" . $perusid . "," . $telkomidku . ",'" . $nama . "'," . $tahun . "," . $batch . "," . $biaya .
			",'" . $sppd . "','" . $nominal . "','" . $namabank . "','" . $cabang . "','" . $atasnama . "','" . $nosurat . "','" . $jenisesopku . "','"
			. $tgl . "','" . $norek . "'," . $userid . ",now());";
//echo $sql . "<br>";
	$result = mysqli_query($link,$sql);
	
	$sql1 = "UPDATE telkomdatindo SET statussusulan=1, jenisbayar='TRF' WHERE ID=" . $telkomidku . ";";
	$result = mysqli_query($link,$sql1);
	
// Log trail -------------------------------
	$dataku = "TELKOMSUSULAN|nama:" . $nama . "|tahun:" . $tahun . "|batch:" . $batch . "|sppd:" . $sppd . "|" . $nominal . "|" . $namabank . "|" .
