...
// Write to Excel
	...
	if ($jmldatamain > 0)
	{
		...
		//$tahunku = 2021;
		$filenameku = './OUTPUT/LKPBU_PASARMODAL_' . strtoupper($bulanarr[$bulanku]) . "_" . $tahunku . 'v1.xlsx';
		$namafileku = 'LKPBU_PASARMODAL_' . strtoupper($bulanarr[$bulanku]) . "_" . $tahunku . 'v1.xlsx';
...
// Change the file
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C3', 'SEC NAME');
		$objPHPExcel->getActiveSheet()->setCellValue('D3', 'FREQ RECEIVE');
		$objPHPExcel->getActiveSheet()->setCellValue('E3', 'TOTAL QTY RECEIVE');
		$objPHPExcel->getActiveSheet()->setCellValue('F3', 'TOTAL PROCEED RECEIVE');
		$objPHPExcel->getActiveSheet()->setCellValue('G3', 'SEC NAME');
		$objPHPExcel->getActiveSheet()->setCellValue('H3', 'FREQ DELIVER');
		$objPHPExcel->getActiveSheet()->setCellValue('I3', 'TOTAL QTY DELIVER');
		$objPHPExcel->getActiveSheet()->setCellValue('J3', 'TOTAL PROCEED DELIVER');
		$objPHPExcel->getActiveSheet()->setCellValue('K3', 'CURR');
...
//echo $scode[$n] . "<br>";
				for ($p=1; $p<=$jmlcode; $p++)
				{
					if ($jmlrec[$n][$p]>0 || $jmldel[$n][$p]>0)
					{
						...
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(1, $pn3, $tglkux[$n]);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(2, $pn3, $scode[$p]);
						if ($jmlrec[$n][$p]==0)
						{
							$jmlrec[$n][$p] = "-";
						}
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(3, $pn3, $jmlrec[$n][$p], PHPExcel_Cell_DataType::TYPE_STRING);
						if ($qtyrec[$n][$p]==0)
						{
							$qtyrec[$n][$p] = "-";
						}
						...
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(5, $pn3, floor($netrec[$n][$p]), PHPExcel_Cell_DataType::TYPE_STRING);
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(6, $pn3, $scode[$p]);
						if ($jmldel[$n][$p]==0)
						{
							$jmldel[$n][$p] = "-";
						}
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(7, $pn3, $jmldel[$n][$p]);
						if ($qtydel[$n][$p]==0)
						{
							$qtydel[$n][$p] = "-";
						}
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(8, $pn3, $qtydel[$n][$p]);
						if ($netdel[$n][$p]==0)
						{
							$netdel[$n][$p] = "-";
						}
						
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(9, $pn3, floor($netdel[$n][$p]));
						$objPHPExcel->getActiveSheet()->SetCellValueByColumnAndRow(10, $pn3, $currx[$n][$p]);
					}
				}
		}
// Write the file
